...
權限響應機制：透過 rememberLauncherForActivityResult 實作 Runtime Permissions 請求。
mapProperties 使用 remember(permissionsGranted)，當定位授權狀態變更時，地圖層會自動切換 isMyLocationEnabled 狀態，確保系統安全性與效能平衡。
...
3. GIS 地理資訊處理與異步調度 (GIS & Concurrency)
...
4. 智慧路網渲染與視覺化邏輯 (Visualization Logic)
...
6. 使用者體驗與情境感知識別 (UX & Context Awareness)
