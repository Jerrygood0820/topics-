...
1.資料擷取層：透過爬蟲 (Crawler) 介接桃園交通局 API 與使用者即時回報。
2.空間計算層：利用 PostGIS 進行空間 Join 與座標路網配對。
3.核心邏輯層 (AI Engine)：執行壅塞分數演算法與感知器驗證邏輯。
4.API 服務層：使用 FastAPI 封裝 JSON 數據分發。
5.客戶端展示層：基於 Jetpack Compose 實作響應式 Android 介面。
